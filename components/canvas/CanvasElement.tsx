...
> = {
  stage:        { background: '#3b82f6', borderRadius: '12px', textColor: 'text-white', Icon: Presentation },
  table:        { background: '#f59e0b', borderRadius: '12px', textColor: 'text-white', Icon: MdTableRestaurant        },
  ...
  entrance:     { background: '#22c55e', borderRadius: '12px', textColor: 'text-white', Icon: DoorOpen     },
  exit:         { background: '#ef4444', borderRadius: '12px', textColor: 'text-white', Icon: DoorClosed   },
  restroom:     { background: '#475569', borderRadius: '12px', textColor: 'text-white', Icon: FaRestroom         },
  bar:          { background: '#f97316', borderRadius: '12px', textColor: 'text-white', Icon: Wine         },
  registration: { background: '#06b6d4', borderRadius: '12px', textColor: 'text-white', Icon: ClipboardList},
}
      ...
      for (const other of others) {
          ...
          for (const [myEdge, offset] of myEdges) {
            for (const theirEdge of theirEdges) {
              if (Math.abs(myEdge - theirEdge) < SNAP_THRESHOLD) {
                rawX = theirEdge - offset; snapX = theirEdge; snappedX = true; break
              }
            }
            if (snappedX) break
          }
        ...
        if (!snappedY) {
          ...
          for (const [myEdge, offset] of myEdges) {
            for (const theirEdge of theirEdges) {
              if (Math.abs(myEdge - theirEdge) < SNAP_THRESHOLD) {
                rawY = theirEdge - offset; snapY = theirEdge; snappedY = true; break
              }
            }
            if (snappedY) break
          }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isSmall && (
          <div
            className={cn('pointer-events-none w-full font-medium leading-tight', config.textColor)}
